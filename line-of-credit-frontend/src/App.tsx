...
function App() {
  ...
  const [repaymentAmounts, setRepaymentAmounts] = useState<{ [key: string]: number }>({})
  ...
  const createTestApplication = async () => {
    ...
    try {
      ...
      const randomTip = Math.floor(Math.random() * 100); // Random tip between 0 and 100
      await axios.post(`${API_BASE_URL}/applications`, {
        userId,
        requestedAmount: randomAmount,
        expressDelivery: Math.random() > 0.5, // 50% chance of express delivery
        tip: randomTip
      });
  ...
  const repayFunds = async (applicationId: string) => {
    const repaymentAmount = repaymentAmounts[applicationId] || 0
    ...
    try {
      ...
      // Clear the repayment amount for this application
                        ...
                        <div className="space-x-2">
                          ...
                          {application.status === ApplicationStatus.OUTSTANDING && (
                            <div className="flex items-center space-x-2">
                              <input
                                type="number"
                                value={repaymentAmounts[application._id] || ''}
                                onChange={(e) => setRepaymentAmounts(prev => ({
                                  ...prev,
                                  [application._id]: Number(e.target.value)
                                }))}
                              ...
                              <button
                                ...
                                disabled={loading || !repaymentAmounts[application._id]}
